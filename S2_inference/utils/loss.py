...
def dtw_loss(ecg1, ecg2): # to do: plot the curve of x-y axis.
    """
    Compute the Dynamic Time Warping (DTW) loss between two ECG sequences.

    Parameters:
    - ecg1: First ECG sequence with shape (batch_size, seq_len1, num_features).
    - ecg2: Second ECG sequence with shape (batch_size, seq_len2, num_features).

    Returns:
    - dtw_loss: DTW loss as a scalar tensor.
    """
    batch_size, seq_len1, num_features = ecg1.size()
    _, seq_len2, _ = ecg2.size()

    # Compute the distance matrix between the two ECG sequences.
    distance_matrix = torch.cdist(ecg1, ecg2)  # Shape: (batch_size, seq_len1, seq_len2).
...
def evaluate_pointcloud(predictions, target, partial_input, n_classes=3):
    ...
    # print(predictions.shape, targets.shape) # torch.Size([1, 3, 4096]) torch.Size([1, 3, 4096])
...
def evaluate_AHA_score(predictions, target, partial_input, n_classes=3):
    '''calculate and compare the covered AHA IDs and the centered AHA ID of prediction and ground truth''' 
    AHA_id = partial_input[:, 7, :].squeeze(0)
    PC_xyz = partial_input[:, 0:3, :].permute(0, 2, 1).squeeze(0)
    targets = F.one_hot(target, n_classes).permute(0, 2, 1)
    ...
    AHA_list_pre, AHA_list_gd = torch.unique(AHA_id[predictions[1]]), torch.unique(AHA_id[targets[1]])
    AHA_loc_score = evaluate_AHA_localization(AHA_center_pre, AHA_list_pre, AHA_center_gd, AHA_list_gd, center_distance)
    return center_distance, AHA_loc_score

def evaluate_pointcloud_new(predictions, target, partial_input, n_classes=3):
    # To address the issue of class imbalance and obtain a more comprehensive evaluation of model performance, 
    # you may consider using other metrics such as precision, recall (or sensitivity), F1-score, and area under the
    # receiver operating characteristic (ROC) curve. These metrics provide a more nuanced evaluation of model performance, 
    # taking into account both true positive and false positive/negative rates for each class separately.
    ...
    try:
        ...
        roc_auc = [0.0 for i in range(n_classes)]
    ...
    return precision, recall, f1_score, roc_auc,specificity, MI_size_pre, MI_size_gd

def evaluate_pointcloud_new_onlyScar(predictions, target,partial_input, n_classes=2):
    
    targets = F.one_hot(target, n_classes).permute(0, 2, 1)  #shape: (1, 2, N)

    """Function to evaluate point cloud predictions with multiple classes"""
    # Convert predictions and targets to boolean values based on threshold
    # predictions = torch.ge(predictions, threshold).bool()
    assert predictions.shape[0] == 1, "Batch size must be 1"
    predictions = one_hot_argmax(predictions).bool()    # (n_classes, N)

    if n_classes==2:
        # prdictions， merge class 1 and 2 as scar
...
def calculate_fusion_loss(y_MI, gt_MI_label, partial_input):
    PC_xyz = partial_input[:, 0:3, :]
    PC_tv =  torch.where((partial_input[:, 7, :] == 0.0) & (partial_input[:, 6, :] > 0), 1, 0)   # AHA == 0, AND cobiveco.tv>0
    ...
    return loss_CE + 0.1*loss_Dice, loss_compactness, loss_MI_RVpenalty, loss_MI_size
...
def calculate_MI_distribution_loss(y_MI_label, gt_MI_label, PC_xyz, PC_tv):
    """
    Compute the compactness of point-cloud data.
    
    Args:
        point_cloud: Point-cloud data with shape (B, N, 3); only works when B=1.
    
    Returns:
        compactness: Compactness of the point-cloud data.
    """
...
def calculate_Dice_onlyScar(inputs, target, num_classes=2):
    
    target_onehot = F.one_hot(target, num_classes).permute(0, 2, 1) # shape: (B, 2, N)
    
    eps = 1e-6
    # Scar: 1, 2
    predictions_scar = inputs[ :,[1],: ] + inputs[ :,[2],: ]
    intersection_scar = torch.sum(predictions_scar * target_onehot[:,[1],:], dim=[0, 2])
    cardinality_scar = torch.sum(predictions_scar + target_onehot[:,[1],:], dim=[0, 2])
    Dice_scar = (2.0 * intersection_scar + eps) / (cardinality_scar + eps)
    
    # norm: 0
    predictions_norm = inputs[ :,[0],: ]
    intersection_norm = torch.sum(predictions_norm * target_onehot[:,[0],:], dim=[0, 2])
    cardinality_norm = torch.sum(predictions_norm + target_onehot[:,[0],:], dim=[0, 2])
    Dice_norm = (2.0 * intersection_norm + eps) / (cardinality_norm + eps)
    Dice = torch.cat((Dice_norm, Dice_scar), dim=0)
...
def one_hot_argmax(input_tensor):
    ...
    """
    input_tensor = input_tensor.permute(0, 2, 1).squeeze(0)  # （4096， 3）
