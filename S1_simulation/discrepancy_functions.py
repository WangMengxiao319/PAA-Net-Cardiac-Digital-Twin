...
def calculate_ecg_errors(predicted_ecg_list, target_ecg, error_method):
    """
    Calculate ECG errors using various methods, adapted for Windows compatibility.
    """
    def calculate_pcc_parallel(target_ecg_pcc, predicted_ecg_list, result_pcc):
        threads_num = multiprocessing.cpu_count()

        def compute_pcc(sample_i):
            return sample_i, np.mean(calculate_ecg_pcc(target_ecg_pcc, predicted_ecg_list[sample_i, :, :]))

        with ThreadPoolExecutor(max_workers=min(threads_num, result_pcc.shape[0])) as executor:
            results = executor.map(compute_pcc, range(result_pcc.shape[0]))

        for sample_i, pcc in results:
            result_pcc[sample_i] = pcc

    def calculate_pcc_2d_parallel(target_ecg_pcc, predicted_ecg_list, result_pcc):
        threads_num = multiprocessing.cpu_count()

        def compute_pcc_2d(sample_i):
            return sample_i, calculate_ecg_pcc(target_ecg_pcc, predicted_ecg_list[sample_i, :, :])

        with ThreadPoolExecutor(max_workers=min(threads_num, result_pcc.shape[0])) as executor:
            results = executor.map(compute_pcc_2d, range(result_pcc.shape[0]))
    ...
    if error_method == 'rmse':
        ...
        result_rmse = calculate_ecg_rmse(predicted_ecg_list, target_ecg)
        target_ecg_pcc = target_ecg[:, :predicted_ecg_list.shape[2]]
        result_pcc = np.full(predicted_ecg_list.shape[0], fill_value=np.nan, dtype=np.float64)
        calculate_pcc_parallel(target_ecg_pcc, predicted_ecg_list, result_pcc)
    ...
    elif error_method == 'rmse_pcc_cubic':
        result_rmse = calculate_ecg_rmse(predicted_ecg_list, target_ecg)
        target_ecg_pcc = target_ecg[:, :predicted_ecg_list.shape[2]]
        result_pcc = np.full((predicted_ecg_list.shape[0], target_ecg.shape[0]), fill_value=np.nan, dtype=np.float64)
        calculate_pcc_2d_parallel(target_ecg_pcc, predicted_ecg_list, result_pcc)

        result_pcc_cubic = np.mean((1.0 - result_pcc) ** 2, axis=1)
        result_pcc_aux = 100 * (result_pcc_cubic)
        result_rmse_aux = 2 * result_rmse / (np.abs(np.amax(target_ecg)))
        result_rmse_pcc = result_pcc_aux + result_rmse_aux
        result_error = result_rmse_pcc

    elif error_method == 'rmse_pcc_fudged':
        fudge_factor_for_simulated_ecg = 0.7
        result_rmse = calculate_ecg_rmse(predicted_ecg_list * fudge_factor_for_simulated_ecg, target_ecg)
        target_ecg_pcc = target_ecg[:, :predicted_ecg_list.shape[2]]
        result_pcc = np.full((predicted_ecg_list.shape[0], target_ecg.shape[0]), fill_value=np.nan, dtype=np.float64)
        calculate_pcc_2d_parallel(target_ecg_pcc, predicted_ecg_list, result_pcc)
        ...
        raise ValueError(f"ECG error method not found: {error_method}")
...
def dtw_ecg_parallel(predicted_ecg_list, target_ecg, max_slope, mesh_volume, w_max):
    """
    Dynamic Time Warping (DTW) distance specific for comparing electrocardiogram signals.
    Adapted for Windows compatibility.
    """
    small_c = 0.05 * 171 / mesh_volume  # Scaling factor to account for differences in mesh size
    p = np.copy(predicted_ecg_list[:, 0, :])
    p[np.isnan(p)] = np.inf
    p, particle_index, unique_index = np.unique(p, return_index=True, return_inverse=True, axis=0)
    p = None
    ...
    # Number of particles and leads
    nParts = predicted_list.shape[0]
    nLeads = predicted_list.shape[1]

    # Initialize results array
    res = np.full(nParts, fill_value=np.nan, dtype=np.float64)
    threads_num = multiprocessing.cpu_count()

    # Function to calculate DTW for a single configuration
    def compute_dtw(conf_i):
        part_dtw = 0.0
        mask = np.logical_not(np.isnan(predicted_list[conf_i, :, :]))
        pred_ecg = np.squeeze(predicted_list[conf_i:conf_i + 1, :, mask[0, :]])
        n_timestamps_1 = pred_ecg.shape[1]
        n_timestamps_2 = target_ecg.shape[1]

        # Compute the trianglogram region
        max_slope_ = max_slope
        min_slope_ = 1 / max_slope_
        scale_max = (n_timestamps_2 - 1) / (n_timestamps_1 - 2)
        max_slope_ *= scale_max
        scale_min = (n_timestamps_2 - 2) / (n_timestamps_1 - 1)
        min_slope_ *= scale_min
        lower_bound = np.floor(min_slope_ * np.arange(n_timestamps_1))
        upper_bound = np.ceil(max_slope_ * np.arange(n_timestamps_1) + 1)
        region_original = np.clip(
            np.asarray([lower_bound, upper_bound]).astype(int), 0, n_timestamps_2
        )
        ...
        for lead_i in range(nLeads):
            ...
            dtw_dist /= np.amax(np.abs(y)) * np.amax(np.abs(target_ecg))
    ...
    with ThreadPoolExecutor(max_workers=min(threads_num, nParts)) as executor:
        executor.map(compute_dtw, range(nParts))
...
def calculate_r_s_sr_rs_wave_progression_healthy_score_population(max_lat_list, predicted_ecg_list):
    """
    Calculate healthy score progression for R-wave, S-wave, S > R, and R > S across a population.
    Adapted for Windows compatibility.
    """
    # Initialize result arrays
    r_wave_progression_healthy_score_list = np.full(predicted_ecg_list.shape[0], fill_value=np.nan, dtype=np.float64)
    s_wave_progression_healthy_score_list = np.full(predicted_ecg_list.shape[0], fill_value=np.nan, dtype=np.float64)
    s_larger_r_progression_healthy_score_list = np.full(predicted_ecg_list.shape[0], fill_value=np.nan, dtype=np.float64)
    r_larger_s_progression_healthy_score_list = np.full(predicted_ecg_list.shape[0], fill_value=np.nan, dtype=np.float64)

    threads_num = multiprocessing.cpu_count()

    # Function to compute healthy scores for a single sample
    def compute_healthy_scores(sample_i):
        r_wave_progression_index, s_wave_progression_index, s_larger_r_progression_index, r_larger_s_progression_index = \
            calculate_r_s_sr_rs_wave_progression(
                max_lat=max_lat_list[sample_i],
                predicted_ecg=predicted_ecg_list[sample_i, :, :]
            )
        r_wave_progression_healthy_score = calculate_r_wave_progression_healthy_score(
            r_wave_progression_index=r_wave_progression_index
        ...
        return (
            sample_i,
            r_wave_progression_healthy_score,
            s_wave_progression_healthy_score,
            s_larger_r_progression_healthy_score,
            r_larger_s_progression_healthy_score
        )

    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, predicted_ecg_list.shape[0])) as executor:
        results = executor.map(compute_healthy_scores, range(predicted_ecg_list.shape[0]))
    ...
    return (
        r_wave_progression_healthy_score_list,
        s_wave_progression_healthy_score_list,
        s_larger_r_progression_healthy_score_list,
        r_larger_s_progression_healthy_score_list
    )
...
def calculate_ecg_qrs_nb_peak_healthy_score_population(max_lat_list, predicted_ecg_list):
    """
    Calculate QRS peak healthy score population for ECG data.
    Adapted for Windows compatibility.
    """
    # Calculate QRS peak per lead
    qrs_nb_peak_pos_neg_per_lead_list = calculate_ecg_qrs_nb_peak_pos_neg_population(
        max_lat_list=max_lat_list, predicted_ecg_list=predicted_ecg_list
    )
    qrs_nb_positive_peak_healthy_range = get_qrs_nb_positive_peak_healthy_range()
    qrs_nb_negative_peak_healthy_range = get_qrs_nb_negative_peak_healthy_range()
    qrs_nb_peak_healthy_range = get_qrs_nb_peak_healthy_range()
    ...
    nb_peak_healthy_score_population = np.zeros(
        (predicted_ecg_list.shape[0], predicted_ecg_list.shape[1]), dtype=np.float64
    )
    threads_num = multiprocessing.cpu_count()

    # Function to compute healthy scores for a single lead
    def compute_healthy_scores(lead_i):
        return lead_i, \
            calculate_range_discrepancy(
                predicted_list=qrs_nb_peak_pos_neg_per_lead_list[:, lead_i, 0],
                good_range=qrs_nb_positive_peak_healthy_range
    ...
    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, predicted_ecg_list.shape[1])) as executor:
        results = executor.map(compute_healthy_scores, range(predicted_ecg_list.shape[1]))

    # Collect results
    for lead_i, healthy_scores in results:
        nb_peak_healthy_score_population[:, lead_i] = healthy_scores
