...
def remove_invalid_and_make_list_of_array(mat):
    """
    Convert each node path into a list of arrays.
    Automatically supports input with shape (N, L) or (N, S, L).
    """
    import numpy as np
    from utils import get_nan_value

    lst = []
    for i in range(mat.shape[0]):
        # Handle dimensions automatically.
        if mat.ndim == 3:
            arr = mat[i, 0, :]   # Select source 0.
        elif mat.ndim == 2:
            arr = mat[i, :]      # Select the entire row directly.
        else:
            raise ValueError(f"Unexpected array shape: {mat.shape}")

        # Remove invalid values (-1 or NaN).
        arr = np.array(arr)
        valid = arr[(arr >= 0) & (arr != get_nan_value())]
        lst.append(valid.astype(np.int32))
    return lst
...
def from_meta_index_list_to_index(meta_index_list, index):
    # Assume meta_index_list is List[np.ndarray], where each element stores a path's meta indices.
    for meta_index_i in range(len(meta_index_list)):
        arr = meta_index_list[meta_index_i]
        if arr.size == 0:
            continue
        mask = (arr >= 0) & (arr < index.shape[0])   # Key step: filter invalid indices.
        meta_index_list[meta_index_i] = index[arr[mask]]
...
#     #     for node_i in range(points_to_map_xyz.shape[0]):
#     # with pymp.Parallel(min(threadsNum, points_to_map_xyz.shape[0])) as p1:
#     #     for node_i in tqdm(p1.range(points_to_map_xyz.shape[0]), desc='Mapping indexes'):
#     #         mapped_indexes[node_i] = np.argmin(
#     #             np.linalg.norm(reference_points_xyz - points_to_map_xyz[node_i, :], ord=2, axis=1)).astype(int)
#     #         error_cumm[node_i] = np.amin(
#     #             np.linalg.norm(reference_points_xyz - points_to_map_xyz[node_i, :], ord=2, axis=1)).astype(int)
#     # TODO: Use a faster approach.
#     # Build a KDTree (implemented in C and very fast).
#     print(f"Building KDTree for {reference_points_xyz.shape[0]} reference points ...")
#     tree = cKDTree(reference_points_xyz)

#     # Query the nearest neighbor.
#     print(f"Querying nearest neighbors for {points_to_map_xyz.shape[0]} points ...")
#     distances, indices = tree.query(points_to_map_xyz, k=1, workers=min(threadsNum, os.cpu_count()))
#     mapped_indexes = indices.astype(int)
#     error_cumm = distances.astype(int)
...
def map_indexes(points_to_map_xyz, reference_points_xyz, error_tolerance=0.5):
    """
    The error tolerance is set assuming that all data is in cm units, 
    which should be consistent in all scripts as of March 2024.
    """
    threadsNum = multiprocessing.cpu_count()

    # Initialize shared arrays
    mapped_indexes = np.zeros(points_to_map_xyz.shape[0], dtype=int)
    error_cumm = np.full(points_to_map_xyz.shape[0], get_nan_value(), dtype=float)

    # Function to compute the mapping for a single node
    def compute_mapping(node_i):
        distances = np.linalg.norm(reference_points_xyz - points_to_map_xyz[node_i, :], ord=2, axis=1)
        mapped_index = np.argmin(distances).astype(int)
        error = np.amin(distances).astype(float)
        return node_i, mapped_index, error

    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threadsNum, points_to_map_xyz.shape[0])) as executor:
        results = executor.map(compute_mapping, range(points_to_map_xyz.shape[0]))

    # Collect results
    for node_i, mapped_index, error in results:
        mapped_indexes[node_i] = mapped_index
        error_cumm[node_i] = error

    # Calculate mean error
    mean_error = np.mean(error_cumm)
    print('mean_error ', mean_error)

    # Check if mean error exceeds tolerance
    if mean_error > error_tolerance:
        raise Exception(
            "The two sets of points don't overlap enough in space, there may be a mistake in the input data."
        )
...
def safe_full(shape, fill_value, dtype):
    """Create an array safely, replacing a NaN fill value with -1 for integer arrays."""
