...
from io_functions import read_csv_file, get_electrode_filename, get_material_filename, \
    ...
    read_scar_border, get_scar_filename, get_bz_filename
import os
...
class RawVCFibreCardiacGeoTet(RawVCCardiacGeoTet):
    def __init__(self, conduction_system, geometric_data_dir, resolution, subject_name, vc_name_list, verbose):
        ...
        self.unprocessed_tetra_material = np.ones((self.unprocessed_tetra.shape[0]), dtype=int)
        # Clear Arguments to prevent Argument recycling
        self.data_path = None
        self.file_prefix = None
...
#     return smoothed_field_data
class EikonalGeometry_withScar(EikonalGeometry):
    ''' This class extends the EikonalGeometry to include scar information
    '''
    def __init__(self, cellular_model, celltype_vc_info, conduction_system, geometric_data_dir, resolution, subject_name, vc_name_list, 
                 verbose):
        super().__init__(cellular_model=cellular_model, celltype_vc_info=celltype_vc_info, conduction_system=conduction_system,
        ...
        self.isScar_BZ = np.zeros(self.edge.shape[0], dtype=int)
        self.scar_name = None
        self.scarNodesIndexes = None
        self.bzNodesIndexes = None

    def load_scar(self, scar_data_dir, scar_name):
        # Contains both border-zone and scar information.
        print(f'loading scar data: {scar_name} .....')

        scarNodes_file_path = get_scar_filename(
            scar_data_dir=scar_data_dir,
            anatomy_subject_name=self.subject_name,
            resolution=self.resolution,
            scar_name=scar_name
        )
        bzNodes_file_path = get_bz_filename(
            scar_data_dir=scar_data_dir,
            anatomy_subject_name=self.subject_name,
            resolution=self.resolution,
            scar_name=scar_name
        ...
        self.scarNodesIndexes = read_scar_border(scarNodes_file_path)
        self.bzNodesIndexes = read_scar_border(bzNodes_file_path)

        isScar = np.all(np.isin(self.edge, self.scarNodesIndexes), axis=1)
        isBZ   = np.all(np.isin(self.edge, self.bzNodesIndexes), axis=1)

        self.isScar_BZ = isScar.astype(int) + 2 * isBZ.astype(int)
        self.scar_name = scar_name
    def load_real_scar(self, scar_data_dir, scar_name):
        # Contains scar information only, without border-zone information.
        print(f'loading real scar data: {scar_name} .....')

        scarNodes_file_path = os.path.join(scar_data_dir, f'{self.subject_name}_{self.resolution}_{scar_name}_scarNodes.csv')
        # IMMC001_20150716_merge_000_coarse_real_scarNodes.csv


        self.scarNodesIndexes = read_scar_border(scarNodes_file_path)
        # self.bzNodesIndexes = read_scar_border(bzNodes_file_path) #TODO：now，real scar doesn't have BZ， future work can be done to add BZ information to the real scar

        isScar = np.all(np.isin(self.edge, self.scarNodesIndexes), axis=1)
        # isBZ   = np.all(np.isin(self.edge, self.bzNodesIndexes), axis=1)

        # self.isScar_BZ = isScar.astype(int) + 2 * isBZ.astype(int)
        self.isScar_BZ = isScar.astype(int)
        self.scar_name = scar_name
...
def generate_scar():
    ...
    foldname = '/path/to/dataset/data_cobiveco_mesh/'
