'''Simulate pipeline of QT using Eikonal model on DHlab geometries. with scar'''
...
from geometry_functions import EikonalGeometry_withScar
from propagation_models import EikonalDjikstraTet_withScar
...
from utils import get_vc_rt_name, \
    ...
    get_vc_aprt_name, get_vc_rvlv_name, get_vc_tm_name, get_vc_ab_name
...
def main(anatomy_subject_name, APD_paramaters, AP_type='MS', is_chronic=True,
         root_dir='/path/to/dataset/',scar_setting = 'real'):
    '''
    anatomy_subject_name: str, e.g. 'DTI004'
    APD_paramaters: list, e.g. [278, 190, 0., 0., 0., 0.]
    '''
    
    t_start = time.time()
    hyperparameter_dict = {}
    hyperparameter_dict['anatomy_subject_name'] = anatomy_subject_name
    ...
    ecg_subject_name = anatomy_subject_name
    # General settings:
    ...
    clinical_data_filename = 'clinical_data/DTI024_clinical_full_ecg.csv'
    clinical_data_filename_path = data_dir + clinical_data_filename
    geometric_data_dir = os.path.join(root_dir, 'DHlab_valid/')

    # Scar settings:
    scar_setting = scar_setting  # choose between 'synthesised' ,'healthy' and 'real'，'real_coarse'
    # ------ Synthesised scar data dir ------
    if scar_setting == 'synthesised':
        scar_data_dir = os.path.join(root_dir, 'DHlab_valid_scar_new_v1/')
    # ------- Real scar data dir ------
    elif scar_setting == 'real':
        scar_data_dir = os.path.join(root_dir, 'DHlab_valid_real_scar/')
    else:  # healthy
        scar_data_dir = None

    # Output Paths:
    if is_chronic:
        experiment_type = 'generate_dataset_DHlab_data_Scar_CV_APD_chronic_new_v1'
    else:
        experiment_type = 'generate_dataset_DHlab_data_Scar_CV_APD_acute'
    # AP_type = 'ToROrd'  # choose the action potential model, ToROrd or MitchellSchaefferEP
    hyperparameter_dict['AP_type'] = AP_type
    ...
    results_dir_root = 'output/'
    if not os.path.exists(results_dir_root):
        os.mkdir(results_dir_root)
    results_dir_part = results_dir_root + experiment_type +'/'
    ...
    # results_dir = results_dir_part + f'APD_params_{index}/'
    ...
    # vc_name_list = [vc_ab_cut_name,vc_rt_name,vc_tm_name]
    vc_name_list = [vc_ab_cut_name, vc_rt_name,  vc_tm_name]
    ...
    geometry = EikonalGeometry_withScar(cellular_model=cellular_model, celltype_vc_info=celltype_vc_info,
    ...
    if scar_setting == 'synthesised':
        ...
        for loc in MI_location_list:
            for extent in MI_transmural_extent_list:
                scar_name = loc + '_' + extent
                scar_name_list.append(scar_name)
    elif scar_setting == 'healthy':
        scar_name_list = ['healthy']
    elif scar_setting == 'real':
        scar_name_list = ['real']  
    for scar_name in scar_name_list:
        hyperparameter_dict['scar_name'] = scar_name
        results_dir = results_dir_part + f'{scar_name}/' 
            
        if not os.path.exists(results_dir):
            os.makedirs(results_dir, exist_ok=True)
        # save ecg
        # visualisation_dir = results_dir + 'checkpoint/'
        visualisation_dir = results_dir
        ...
        if scar_name != 'healthy' and scar_name != 'real':
            print('Loading scar data from ', scar_data_dir)
            geometry.load_scar(scar_data_dir=scar_data_dir, scar_name=scar_name)
        elif scar_name == 'real':
            print('Loading real scar data from ', scar_data_dir)
            geometry.load_real_scar(scar_data_dir=scar_data_dir, scar_name=scar_name)
        ...
        scar_scale = 0.1
        border_zone_scale = 0.5
        
        propagation_model = EikonalDjikstraTet_withScar(
            ...
            sheet_speed_name=sheet_speed_name, scar_scale = scar_scale, border_zone_scale = border_zone_scale, verbose=verbose)
        ...
        if is_chronic:
            apd_bz_nz_ratio = 1.3 #  Chronic MI:1.1-1.3, ACute MI: 0.5-0.8
        else:
            apd_bz_nz_ratio = 0.5
        hyperparameter_dict['apd_bz_nz_ratio'] = apd_bz_nz_ratio
        electrophysiology_model = ElectrophysiologyAPDmap(apd_max_name=apd_max_name, apd_min_name=apd_min_name,
                                                            ...
                                                            verbose=verbose,
                                                            apd_bz_nz_ratio = apd_bz_nz_ratio
                                                            )
        ...
        electrophysiology_parameter_example = np.array(APD_paramaters)
        hyperparameter_dict['APD_parameters'] = electrophysiology_parameter_example.tolist()
        ...
        # Save the new xyz coordinates and tetrahedra.
        # np.savetxt(os.path.join(visualisation_dir,"xyz.csv"), geometry.get_node_xyz(), delimiter=",")
        # np.savetxt(os.path.join(visualisation_dir,"tetra.csv"), geometry.get_tetra(), delimiter=",")
        # plot_geometry(xyz=geometry.get_node_xyz(), surf=extract_surface(geometry.get_tetra()), lat_simulation=lat_simulation)

        # Save LAT as CSV.
        np.savetxt(os.path.join(visualisation_dir,"lat_simulation.csv"), lat_simulation, delimiter=",")
        
        # # Save VM as CSV.
        np.savetxt(os.path.join(visualisation_dir,"vm_simulation.csv"), vm_simulation, delimiter=",")
        ...
        # Save the ECG.
        np.savetxt(os.path.join(visualisation_dir,"predicted_ecg.csv"), healthy_predicted_ecg, delimiter=",")
        # save hyperparameters
        hyperparameter_record_file =visualisation_dir + 'hyperparameters.json'
        save_dictionary(dictionary=hyperparameter_dict, filename=hyperparameter_record_file)
...
if __name__ == '__main__':
    # anatomy_subject_name_list = ['IMMC001_20150716_merge_000']
    root_dir = '/path/to/dataset/'

    anatomy_subject_name_list = list_subfolders(os.path.join(root_dir, 'DHlab_valid/'))
    # Sort by name.
    anatomy_subject_name_list.sort()
    anatomy_subject_name = 'IMMC049_20170717_merge_000'  # IMMC210_20181126_merge_000 (visualization), IMMC049_20170717_merge_000 (paper comparison experiment)
    
    # Skip subjects that already have result directories.
    # existing_results = list_subfolders('output/generate_dataset_DHlab_data/')
    # anatomy_subject_name_list = [name for name in anatomy_subject_name_list if name not in existing_results]
    print('Number of anatomy_subject_name_list ', len(anatomy_subject_name_list))
    # APD_paramaters_list = [
    #     [330.7, 189.4,  0.4, -0.83, 0.66, 0.0],   # DTI024, APD_param0
    #     [366, 193.6, 0.87, -0.59, 0.79, 0.04],   # DTI004， APD_param1
    #     [317.5, 187.9,  0.64, -0.73, 0.79, 0.15]    # DTI032， APD_param2
    # ]
    # AP_type_list = ['ToROrd','MitchellSchaefferEP']
    
    # error log file
    APD_paramaters =  [330.7, 189.4,  0.4, -0.83, 0.66, 0.0]
    AP_type = 'MitchellSchaefferEP'
    error_log_file = 'output/generate_dataset_DHlab_data_Scar_CV_APD_chronic_new_v1/simulation_error_log.txt'

    # for anatomy_subject_name in anatomy_subject_name_list:
    # try:
    print('Running Scar simulation for subject ',anatomy_subject_name)
    main(
        anatomy_subject_name=anatomy_subject_name,
        APD_paramaters=APD_paramaters,
        AP_type=AP_type,
        root_dir=root_dir,
        scar_setting='real',
    )
    ...
    #         anatomy_subject_name,
    #         ', skipping this subject.'
    #     )

    #     with open(error_log_file, 'a') as f:
    #         f.write(
    #             f'Simulation failed for subject {anatomy_subject_name}\n'
    #         )
    #         f.write(f'Error message: {str(e)}\n')

    #     # Continue directly to the next anatomy_subject_name.
    #     continue
