...
class ElectrophysiologyUpstrokeStepFunction(Electrophysiology):
    ...
    def simulate_electrophysiology_population(self, parameter_population_modules_dict):
        ...
        #                                           simulation_time), dtype=np.float64)
        # vm_population_unique[:, :, :] = get_nan_value() # TODO be careful that there are no NAN values in the ECG calculation
        vm_population_unique = np.full((lat_population_unique.shape[0], lat_population_unique.shape[1],
                                                  simulation_time), get_nan_value(), dtype=np.float64)
        ...
        with ThreadPoolExecutor(max_workers=min(threads_num, vm_population_unique.shape[0])) as executor:
            results = executor.map(simulate_for_config, range(vm_population_unique.shape[0]))

         # Collect results
        for conf_i, vm in results:
            vm_population_unique[conf_i, :, :] = vm
...
class ElectrophysiologySameAP(Electrophysiology):
    ...
    def simulate_electrophysiology_population(self, parameter_population_modules_dict):
        """
        Simulates the electrophysiology population based on parameter modules, modified for Windows compatibility.
        """
        # Simulate propagation and extract unique LAT populations
        ...
        # Initialize Vm population array
        vm_population_unique = np.full(
            (lat_population_unique.shape[0], lat_population_unique.shape[1], simulation_time),
            get_nan_value(),
            dtype=np.float64
        )
        ...
        # Function to simulate Vm for a single configuration
        def simulate_for_config(conf_i):
            vm = np.zeros((lat_population_unique.shape[1], simulation_time))
            vm_simulated = calculate_node_vm_from_any_ap(
                ap=self.action_potential_simulation,
                lat=lat_population_unique[conf_i, :],
                simulation_time=simulation_time,
                upstroke_index=self.upstroke_index
            )
        ...
        # Return LAT population and Vm population with unique indexes restored
        return lat_population, vm_population_unique[unique_inverse_indexes, :, :]
...
class ElectrophysiologyAPDmap(Electrophysiology):
    def __init__(self, apd_max_name, apd_min_name, cellular_model, fibre_speed_name,
        ...
        self.apd_bz_nz_ratio = apd_bz_nz_ratio
    ...
    def simulate_electrophysiology(self, parameter_particle_modules_dict):
        ...
        # )
        vm_simulation = unsmoothed_node_vm
        return lat_simulation, vm_simulation
    ...
    # def simulate_electrophysiology_population(self, parameter_population_modules_dict):
    #     """
    #     Simulates the electrophysiology population based on parameter modules, adapted for Windows compatibility.
    #     """
    #     # Get cell types and simulate LAT population
    #     node_celltype = self.propagation_model.get_node_celltype()
    #     lat_population = self.propagation_model.simulate_propagation_population(
    #         parameter_population_modules_dict=parameter_population_modules_dict
    #     )

    #     # Prepare unique simulation configurations
    #     parameter_population = self.get_from_module_dict(parameter_population_modules_dict)
    #     simulation_configuration_population = np.concatenate((lat_population, parameter_population), axis=1)
    #     simulation_configuration_population_unique, unique_indexes, unique_inverse_indexes = np.unique(
    #         simulation_configuration_population, return_index=True, return_inverse=True, axis=0
    ...
    #     # Initialize Vm population
    #     vm_population_unique = np.full(
    #         (parameter_population_unique.shape[0], lat_population_unique.shape[1], max_simulation_time),
    #         fill_value=np.nan,
    #         dtype=np.float64
    #     )
    #     threads_num = multiprocessing.cpu_count()
    ...
    #     # Function to simulate Vm for a single configuration
    #     def simulate_vm_for_particle(conf_i):
    #         parameter_particle = parameter_population_unique[conf_i]
    #         unsmoothed_node_vm = self.generate_node_vm(
    #             parameter_particle=parameter_particle,
    #             node_lat=lat_population_unique[conf_i],
    #             node_celltype=node_celltype
    #         )
    #         smoothed_vm = self.apply_spatiotemporal_smoothing(nodefield=unsmoothed_node_vm)
    #         return conf_i, smoothed_vm

    #     # Use ThreadPoolExecutor for parallel processing
    #     with ThreadPoolExecutor(max_workers=min(threads_num, parameter_population_unique.shape[0])) as executor:
    #         results = executor.map(simulate_vm_for_particle, range(parameter_population_unique.shape[0]))

    #     # Collect results
    #     for conf_i, smoothed_vm in results:
    #         vm_population_unique[conf_i, :, :smoothed_vm.shape[1]] = smoothed_vm
    ...
    def generate_node_apd(self, parameter_particle):
        ...
        geometry = self.propagation_model.geometry
        ...
        # FIXME: add APD change for border zones if needed
        if geometry.bzNodesIndexes is not None and len(geometry.bzNodesIndexes) > 0:
            normalised_gradient_map[geometry.bzNodesIndexes] = normalised_gradient_map[geometry.bzNodesIndexes] * self.apd_bz_nz_ratio
...
#         node_xyz = np.loadtxt('/path/to/data/geometric_data/sample/sample_coarse/sample_coarse_xyz.csv', delimiter=',')
