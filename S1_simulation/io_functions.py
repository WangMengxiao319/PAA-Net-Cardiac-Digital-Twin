...
def get_geometric_data_path(anatomy_subject_name, geometric_data_dir, resolution):
    ...
    return geometric_data_dir + anatomy_subject_name + '/'
    
def get_scar_data_path(anatomy_subject_name, scar_data_dir, resolution):
    # return geometric_data_dir + anatomy_subject_name + '/' + anatomy_subject_name + '_' + resolution + '/'
    return scar_data_dir + anatomy_subject_name + '/'
    

def get_geometric_file_prefix(anatomy_subject_name, resolution):
    return anatomy_subject_name + '_' + resolution

# Ukbiobank dataset path
def get_ukbiobank_file_path(anatomy_subject_name, ukbiobank_data_dir):
    return ukbiobank_data_dir + anatomy_subject_name + '/' + anatomy_subject_name
...
### Scar & BZ 
def read_scar_border(filename):
    return (np.loadtxt(filename) - 1).astype(int)   
# scar
def get_scar_filename(anatomy_subject_name, scar_data_dir, resolution,scar_name):
    geometric_data_path = get_scar_data_path(anatomy_subject_name, scar_data_dir, resolution)
    geometric_file_prefix = get_geometric_file_prefix(anatomy_subject_name, resolution)
    print('geometric_data_path ', geometric_data_path)
    return geometric_data_path + geometric_file_prefix +  '_new_v1_lvscarnodes_' + scar_name + '.csv'
# bz
def get_bz_filename(anatomy_subject_name, scar_data_dir, resolution,scar_name):
    geometric_data_path = get_scar_data_path(anatomy_subject_name, scar_data_dir, resolution)
    geometric_file_prefix = get_geometric_file_prefix(anatomy_subject_name, resolution)
    return geometric_data_path + geometric_file_prefix +  '_new_v1_lvborderzonenodes_' + scar_name + '.csv'
...
def read_ukbiobank_rn_scar_border(filename):
    return (np.loadtxt(filename) - 1).astype(int)   
...
def write_geometry_to_ensight(geometry, subject_name, resolution, verbose, visualisation_dir):
    ...
    try:
        ...
        node_fibre = geometry.get_node_fibre() # FIXME: 
...
def read_monoalg_vm_ensight(ensight_dir, nb_node):
    ...
    # return vm
    # Initialize the VM array
    vm = np.zeros((vm_0.shape[0], len(filename_list)), dtype=float)

    # Set the first column with provided data
    vm[:, 0] = vm_0

    threads_num = multiprocessing.cpu_count()

    # Function to read a single file
    def read_file(time_i):
        return time_i, read_monoalg_vm_file(
            filename=ensight_dir + filename_list[time_i],
            nb_node=nb_node
        )

    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, len(filename_list) - 1)) as executor:
        results = executor.map(read_file, range(1, len(filename_list)))

    # Collect results
    for time_i, data in results:
        vm[:, time_i] = data
...
def read_time_csv_fields(anatomy_subject_name, csv_dir, file_name_tag, node_xyz_filename):
    """
    Read time-series CSV fields in parallel, adapted for Windows compatibility.
    """
    print("Reading Vm")
    
    # Read time and time indices
    time = pd.read_csv(csv_dir + "timeset_1.csv", delimiter=",", header=None).values.transpose()[0].astype(float)
    assert 0.00099 < time[1] - time[0] < 0.0011, "Time resolution is NOT 1 ms"
    time_index = pd.read_csv(csv_dir + "timeindices_1.csv", delimiter=",", header=None).values.transpose()[0].astype(int)
    assert time.shape == time_index.shape, "Landmine in function read_csv_fields"
    
    # Read node positions
    node_xyz = pd.read_csv(node_xyz_filename, delimiter=",", header=None).values
    
    # Extract filenames
    filenames = np.array(
        [f for f in os.listdir(csv_dir) if os.path.isfile(os.path.join(csv_dir, f)) and (file_name_tag in f)]
    )
    ...
    def read_file(time_i):
        index = "{:06d}".format(time_index[time_i])
        filename = f"{csv_dir}{anatomy_subject_name}.{field_type}.{field_name}-{index}.csv"
        data = pd.read_csv(filename, delimiter=",", header=None).values.transpose()[0]
        return time_i, data

    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, time_index.shape[0])) as executor:
        results = executor.map(read_file, range(time_index.shape[0]))
