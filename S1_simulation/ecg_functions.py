...
def calculate_ecg_biomarker_from_only_ecg(heart_rate, max_lat_list, predicted_ecg_list):
    """
    Calculate ECG biomarkers: corrected qt_dur (QTc), t_pe, t_peak, qtpeak_dur, t_polarity.
    Adapted for Windows compatibility.
    """
    # Constants and initialization
    nb_leads = predicted_ecg_list.shape[1]
    qtc_dur_lead = np.full((predicted_ecg_list.shape[0], nb_leads), fill_value=np.nan, dtype=np.float64)
    t_pe_lead = np.full((predicted_ecg_list.shape[0], nb_leads), fill_value=np.nan, dtype=np.float64)
    t_peak_lead = np.full((predicted_ecg_list.shape[0], nb_leads), fill_value=np.nan, dtype=np.float64)
    qtpeak_dur_lead = np.full((predicted_ecg_list.shape[0], nb_leads), fill_value=np.nan, dtype=np.float64)
    t_polarity_lead = np.full((predicted_ecg_list.shape[0], nb_leads), fill_value=np.nan, dtype=np.float64)

    threads_num = multiprocessing.cpu_count()

    # Function to compute biomarkers for a single sample
    def compute_biomarkers_for_sample(sample_i):
        for lead_i in range(nb_leads):
            qtc_dur, t_pe, t_peak, qtpeak_dur, t_polarity = calculate_ecg_lead_biomarker_from_only_ecg(
                ecg_lead=predicted_ecg_list[sample_i, lead_i, :],
    ...
    with ThreadPoolExecutor(max_workers=min(threads_num, predicted_ecg_list.shape[0])) as executor:
        executor.map(compute_biomarkers_for_sample, range(predicted_ecg_list.shape[0]))

    warnings.warn(
        "The order of the biomarkers is static: qt_dur_lead, t_pe_lead, t_peak_lead, qtpeak_dur_lead, t_polarity_lead"
    )
    return qtc_dur_lead, t_pe_lead, t_peak_lead, qtpeak_dur_lead, t_polarity_lead
...
def calculate_ecg_qrs_axis_population(max_lat_list, predicted_ecg_list):
    """
    Calculate the QRS axis for a population of ECGs, adapted for Windows compatibility.
    """
    qrs_axis_list = np.full((predicted_ecg_list.shape[0]), fill_value=np.nan, dtype=np.float64)
    threads_num = multiprocessing.cpu_count()

    # Function to compute QRS axis for a single sample
    def compute_qrs_axis(sample_i):
        return sample_i, calculate_ecg_qrs_axis(
            max_lat=max_lat_list[sample_i],
            predicted_ecg=predicted_ecg_list[sample_i, :, :]
        )
    ...
    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, predicted_ecg_list.shape[0])) as executor:
        results = executor.map(compute_qrs_axis, range(qrs_axis_list.shape[0]))

    # Collect results
    for sample_i, qrs_axis in results:
        qrs_axis_list[sample_i] = qrs_axis
...
def calculate_ecg_qrs_nb_peak_pos_neg_population(max_lat_list, predicted_ecg_list):
    """
    Calculate the number of positive and negative peaks in QRS for each lead across a population of ECGs.
    Adapted for Windows compatibility.
    """
    qrs_nb_peak_per_lead_list = np.full(
        (predicted_ecg_list.shape[0], predicted_ecg_list.shape[1], 2),
        fill_value=np.nan,
        dtype=np.float64
    )
    threads_num = multiprocessing.cpu_count()

    # Function to compute QRS peaks for a single sample
    def compute_qrs_peaks(sample_i):
        return sample_i, calculate_ecg_qrs_nb_peak_pos_neg(
            max_lat=max_lat_list[sample_i],
            predicted_ecg=predicted_ecg_list[sample_i, :, :]
        )
    ...
    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(threads_num, predicted_ecg_list.shape[0])) as executor:
        results = executor.map(compute_qrs_peaks, range(qrs_nb_peak_per_lead_list.shape[0]))

    # Collect results
    for sample_i, qrs_peaks in results:
        qrs_nb_peak_per_lead_list[sample_i, :, :] = qrs_peaks
...
class PseudoEcgTetFromVM(CalculateEcg):
    ...
    # def calculate_ecg_population(self, lat_population, vm_population):
    #     """
    #     Calculate ECG population based on LAT and Vm population, modified for Windows compatibility.
    #     """
    #     # Truncate vm_population to the maximum length of ECG
    #     vm_population = vm_population[:, :, :self.max_len_ecg]

    #     # Skip the unique logic for now (as in the original code)
    #     vm_population_unique = vm_population
    #     lat_population_unique = lat_population

    #     # Initialize the ECG population array
    #     ecg_population_unique = np.full(
    #         (vm_population_unique.shape[0], self.nb_leads, vm_population_unique.shape[2]),
    #         get_nan_value(),
    #         dtype=np.float64
    #     )
    ...
    #     # Function to calculate ECG for a single configuration
    #     def calculate_for_config(conf_i):
    #         aux_ecg = self.calculate_ecg(
    #             lat=lat_population_unique[conf_i, :],
    #             vm=vm_population_unique[conf_i, :, :]
    #         )
    #         return conf_i, aux_ecg

    #     # Use ThreadPoolExecutor for parallel processing
    #     with ThreadPoolExecutor(max_workers=min(threads_num, ecg_population_unique.shape[0])) as executor:
    #         results = executor.map(calculate_for_config, range(ecg_population_unique.shape[0]))

    #     # Collect results
    #     for conf_i, aux_ecg in results:
    #         ecg_population_unique[conf_i, :, :aux_ecg.shape[1]] = aux_ecg

    #     return ecg_population_unique
    ...
    def visualise_ecg(self, discrepancy_population, ecg_population):#, save_dir=None): # TODO:
