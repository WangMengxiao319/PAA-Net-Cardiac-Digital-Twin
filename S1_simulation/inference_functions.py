...
def sample_nb_root_nodes_population(population_size, theta_boundaries, nb_root_nodes_cdf):
    ...
    for nb_root_nodes_i in tqdm(range(population_size),total= population_size, desc="Sampling number of populations"):
...
class BayesianInferenceSMCABC(SamplingMethod):
    ...
    def sample(self, desired_discrepancy, max_sampling_time, unique_stopping_ratio, visualisation_count,
        ...
        self.population_discrepancy = self.compute_discrepancy(self.population_theta)  # main
        ...
        # inference_history_path = '/path/to/results/personalisation_data/sample/inference_history/'
...
class MixedBayesianInferenceRootNodes(ContinuousSMCABC):
    ...
    def mvn_move(self, c_theta, cuttoff_discrepancy, kept_theta, kept_discrepancies, nb_mcmc_moves, replacement_theta, replacement_discrepancies):
        """
        Adapted mvn_move function to be compatible with Windows, replacing pymp with ThreadPoolExecutor.
        """
        nb_particles_to_jiggle = replacement_theta.shape[0]
        accepted_moves = np.full(shape=(nb_particles_to_jiggle), fill_value=0, dtype=np.bool_)
        ...
        for mcmc_move_i in range(nb_mcmc_moves):
            ...
            # Calculate probabilities for discrete replacement theta
            replacement_theta_discrete_unique, replacement_theta_discrete_unique_indexes = np.unique(
                replacement_theta_discrete, return_inverse=True, axis=0)
            replacement_theta_discrete_unique_probabilities = np.zeros(
                replacement_theta_discrete_unique.shape[0], dtype=np.float64)

            def compute_discrete_probability(index):
                return index, self.compute_probability_discrete(
                    new_binaries=replacement_theta_discrete_unique[index, :], part_binaries=kept_theta_discrete)

            threads_num = multiprocessing.cpu_count()
            with ThreadPoolExecutor(max_workers=min(threads_num, replacement_theta_discrete_unique.shape[0])) as executor:
                results = executor.map(compute_discrete_probability, range(replacement_theta_discrete_unique.shape[0]))

            for index, prob in results:
                replacement_theta_discrete_unique_probabilities[index] = prob

            replacement_theta_discrete_probabilities = replacement_theta_discrete_unique_probabilities[
                replacement_theta_discrete_unique_indexes]
            ...
            proposed_theta_discrete_unique, proposed_theta_discrete_unique_indexes = np.unique(
                proposed_theta_discrete, return_inverse=True, axis=0)
            proposed_theta_discrete_unique_probabilities = np.zeros(
                proposed_theta_discrete_unique.shape[0], dtype=np.float64)

            def compute_proposed_discrete_probability(index):
                return index, self.compute_probability_discrete(
                    new_binaries=proposed_theta_discrete_unique[index, :], part_binaries=kept_theta_discrete)

            with ThreadPoolExecutor(max_workers=min(threads_num, proposed_theta_discrete_unique.shape[0])) as executor:
                results = executor.map(compute_proposed_discrete_probability, range(proposed_theta_discrete_unique.shape[0]))

            for index, prob in results:
                proposed_theta_discrete_unique_probabilities[index] = prob

            proposed_theta_discrete_probabilities = proposed_theta_discrete_unique_probabilities[
                proposed_theta_discrete_unique_indexes]

            # Determine which of the proposed discrete particles are rare enough to be taken instead of the original reference
            non_accepted_proposed_theta_discrete_indexes = np.random.rand(nb_particles_to_jiggle) > replacement_theta_discrete_probabilities / proposed_theta_discrete_probabilities
            proposed_theta_discrete[non_accepted_proposed_theta_discrete_indexes, :] = replacement_theta_discrete[non_accepted_proposed_theta_discrete_indexes, :]
            proposed_theta = np.concatenate((replacement_theta_continuous, proposed_theta_discrete), axis=1)
            proposed_discrepancy = self.compute_discrepancy(proposed_theta)
            accepted_moves = np.logical_or(proposed_discrepancy < cuttoff_discrepancy, accepted_moves)

            # Update replacement particles
            replacement_theta[proposed_discrepancy < cuttoff_discrepancy] = proposed_theta[proposed_discrepancy < cuttoff_discrepancy]
            replacement_discrepancies[proposed_discrepancy < cuttoff_discrepancy] = proposed_discrepancy[proposed_discrepancy < cuttoff_discrepancy]

            # Jiggle continuous parameters
            proposed_theta_continuous = jiggle_continuous_dependent(
                c_theta=c_theta, replacement_theta_continuous=replacement_theta_continuous)
            proposed_theta = np.concatenate((proposed_theta_continuous, replacement_theta[:, self.nb_continuous_theta:]), axis=1)
            proposed_theta = self.adjust_theta_values(proposed_theta)

            temp_accepted_indexes_theta = self.check_theta_constraints(
                boundaries_theta=self.boundaries_continuous_theta, theta_population=proposed_theta_continuous)
            if np.sum(temp_accepted_indexes_theta) > 0:
                pre_accepted_proposed_discrepancy = self.compute_discrepancy(
                    proposed_theta[temp_accepted_indexes_theta, :])
                discrepancy_accepted_indexes = pre_accepted_proposed_discrepancy < cuttoff_discrepancy
                temp_accepted_indexes_theta[temp_accepted_indexes_theta] = discrepancy_accepted_indexes
            ...
            accepted_moves = np.logical_or(temp_accepted_indexes_theta, accepted_moves)
